...
## Вид сверху на раскладку из GeometryLab. Рисует комнаты на сетке: футпринт-клетки
...
## выбор комнаты (клик).
##
## Чисто отладочная проекция (см. docs/geometry-lab.md): продакшн-геометрия — world_generator.gd.
...
## Принимает раскладку из GeometryLab.build и перерисовывает, вписывая в вид.
...
## комнаты могут пересекаться между собой; каждый сам с собой — нет (см. GeometryLab._compute_routes).
...
## Пути связей родитель→ребёнок по ЦЕНТРАМ клеток, в обход комнат (см. GeometryLab._route_corridor).
