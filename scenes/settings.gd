...
func _build_security_tab() -> void:
	...
	var title := Label.new()
	title.text = "WASM-модули страниц"
	title.add_theme_font_size_override("font_size", 18)
	content.add_child(title)
	var behavior := Label.new()
	behavior.text = "Страницы используют WebAssembly Components без системных полномочий по умолчанию. Текущая сборка ещё не содержит WASM runtime: такие компоненты локально пропускаются, а статическая сцена продолжает работать."
