...
@onready var _thumb: ColorRect = $SubViewport/ScrollThumb
...
## Player вызывает это через общий WorldUiSurface-контракт.
func _on_ui_scroll(dir: float) -> void:
	if not _scrollable:
		return
...
## Маппит UV общей world-space поверхности в пиксель вьюпорта и «кликает» там за игрока.
...
func _ui_is_active(uv: Vector2) -> bool:
	var px := canvas_px(uv)
	if px.x < 0.0:
		return false
	_probing = true
...
func _build_quad() -> void:
	...
	configure_canvas_geometry(Vector2(w_m, h_m))
