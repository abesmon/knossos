...
## Режим прицеливания размещения картинки включён/выключен (кнопка 3). main показывает
## подсказку в строке статуса. См. docs/network/realtime-resources.md.
signal image_placement_active(active: bool)
## Место размещения подтверждено (ЛКМ в режиме прицеливания): xform — мировой трансформ
## будущей картинки (позиция + разворот по нормали поверхности / лицом к игроку). main
## открывает файловый диалог и создаёт объект в эфемерном слое.
signal image_placement_confirmed(xform: Transform3D)
...
var _placing := false        # режим прицеливания размещения картинки (кнопка 3)
var _place_preview: MeshInstance3D    # полупрозрачный квадрат «здесь будет картинка»
var _place_last := Transform3D.IDENTITY  # трансформ превью с прошлого физ-кадра (луч — только там)
...
# --- Инструмент размещения картинок (кнопка 3), см. docs/network/realtime-resources.md ---
...
func capture_mouse(on: bool) -> void:
	...
	if on:
		...
		# Мышь отпущена не подтверждением (Esc, потеря фокуса) — выходим из прицеливания.
		_cancel_placement()
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and _looking:
		...
		if _looking:
			...
			if _placing:
				_confirm_placement()
			elif _tool != null and _tool.is_armed():
				_tool.press()
			else:
				_try_interact()
		else:
...
func _setup_place_preview() -> void:
	_place_preview = MeshInstance3D.new()
	_place_preview.name = "ImagePlacePreview"
	_place_preview.top_level = true
	var quad := QuadMesh.new()
	quad.size = Vector2(PLACE_PREVIEW_SIZE, PLACE_PREVIEW_SIZE)
	_place_preview.mesh = quad
	var mat := StandardMaterial3D.new()
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mat.cull_mode = BaseMaterial3D.CULL_DISABLED
	mat.albedo_color = Color(0.4, 0.85, 1.0, 0.35)
	_place_preview.material_override = mat
	_place_preview.visible = false
	add_child(_place_preview)
...
func _start_placement() -> void:
	...
	_place_last = Transform3D(_face_basis(-fwd, fwd), _camera.global_position + fwd * PLACE_REACH)
	image_placement_active.emit(true)
...
func _cancel_placement() -> void:
	...
	_placing = false
	_place_preview.visible = false
	image_placement_active.emit(false)
...
func _confirm_placement() -> void:
	...
	_place_preview.visible = false
	image_placement_active.emit(false)
	image_placement_confirmed.emit(_place_last)


## Физ-кадр: пересчитать трансформ по лучу и подвинуть/показать превью.
func _update_place_preview() -> void:
	_place_last = _compute_placement()
	_place_preview.global_transform = _place_last
...
## Зовётся только из _physics_process (запрос к direct_space_state безопасен там).
func _compute_placement() -> Transform3D:
	var origin := _camera.global_position
	var fwd := -_camera.global_transform.basis.z
	var to := origin + fwd * PLACE_REACH
	var query := PhysicsRayQueryParameters3D.create(origin, to, PLACE_MASK, [get_rid()])
...
func _physics_process(delta: float) -> void:
	...
	if _debug_on:
		_debug_probe()
	if _placing:
		# Луч размещения — только в физ-кадре (direct_space_state, как у прокрутки/инспектора).
