...
func _update_layout() -> void:
	...
	# Квад стоит на полу и растёт вверх: корень эктора — на уровне пола.
	_mesh.position = Vector3(0, _height_m * 0.5, 0)
	_shape.size = Vector3(_quad.size.x, _height_m, 0.08)
	_collision.position = Vector3(0, _height_m * 0.5, 0)
	_label.position = Vector3(0, _height_m * 0.5, 0.06)
