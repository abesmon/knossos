...
export template, экспортирует Knossos и запускает из полученного приложения lifecycle реального
`test_pages/lights.vrmod`. macOS exported smoke локально подтверждён: runtime dylib присутствует,
