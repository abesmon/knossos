...
> **Статус:** текущий контракт обновлён 21 июля 2026: общий Store/RPC, Subject Bindings и
> видео-адаптер работают; SceneChanges и grabbable переведены на тот же binding contract.
> Ручной тест двух экземпляров подтвердил
...
>
> **Вердикт:** взять у VRChat разделение на **состояние, события и назначенного controller**,
> но выразить controller широким named binding, а не отдельной сущностью owner, и не строить
...
>
> **Назначения:** record содержит `bindings`; predicate `assigned(slot)` и reducer-результат
> `{state, bindings}` покрывают creator/holder/presenter и мировые роли. Authority остаётся
> единственным сериализатором. См. [subject-bindings.md](subject-bindings.md).
...
Для VRWeb правильная граница: общий transport/envelope — да; один публичный
state engine для аватаров, видео, графа сцены, чата и эффектов — нет. `SAMPLE`, `DELTA`,
`COMMAND` и `SNAPSHOT` могут использовать общие кодеки, но разные доменные слои
должны явно выбирать, какие из этих режимов им доступны.

Rate limiter здесь относится к policy/защите конкретного клиента Knossos, а не к нормативному
ограничению VRWeb. Стандарт не решает за автора, какая частота или стоимость authored network
behavior допустима; клиент может фильтровать или пропускать поток по своим возможностям.
...
нужно, но вывод для протокола — нужны явный режим доставки и наблюдаемость. Численные пределы и
rate limiting являются решениями конкретной реализации, а не нормой VRWeb.
...
- `SceneChanges` — хорошая чистая машина авторитетного состояния, но её поля `kind`, `parent`,
  `bindings.creator`, `ttl`, `props` нужны именно редактируемому графу;
...
assigned: slot                    requester назначен в binding с данным именем
vacant: slot                      slot не назначен
...
- Snapshot может опередить загрузку страницы и регистрацию её схем. Records неизвестных
  локальному клиенту схем Store временно держит отдельно и материализует при
  `ensure_object` после регистрации схемы; до объявления объекта они не видны потребителям
  и проходят полную field-валидацию только при активации. Выбрасывать их нельзя: глобальный `seq` снимка уже принят,
  а потерянная object revision превращает следующую `DELTA` в gap и оставляет late joiner с
  default-state до повторного resync.
...
ReplicatedStateStore (схемы, authority, revision/epoch/seq, snapshot, лимиты)
        ↓ signals / local API
VideoStateAdapter, RigidbodySync, будущие DoorStateAdapter, SliderStateAdapter…
        ↓
...
`src`, `loop` и `volume` — часть страницы и в стандартную схему не входят **по построению**:
у синтезированного плеера источник задан разметкой и одинаков у всех клиентов сам по себе,
а любая смена источника (`set_source`) исходит из авторского скрипта — и тогда синхронизацию
источника ведёт сам скрипт через собственную схему `document.state`
(см. [video-player.md](../client/video-player.md#архитектура-базовый-уровень-и-надстройки)).
Действия пользователя становятся командами `set_playing`/`seek`, результат — новое состояние
с одной ревизией. Late join получает snapshot сразу, без ожидания следующего heartbeat.
...
1. **Реализованный пилот:** отдельный video-heartbeat заменён generic-сообщением
   ...
   Получаем общий state/late join/bindings без изменения качества синхронизации.
...
## Byte budgets Knossos и snapshot transport

Числа ниже описывают текущую реализацию Knossos. Это не лимиты стандарта VRWeb и не требования
к альтернативным клиентам: они могут принимать, фильтровать, пропускать или иначе обслуживать
authored-поток согласно собственной policy.
...
Не заменять `SceneChanges`: потеряются семантика графа, TTL, каскад и флаш. SceneChanges
использует тот же `bindings.creator` и PolicyEvaluator, но остаётся отдельной доменной машиной.
...
- **Поверхность атаки:** структурная проверка схем и конечности чисел обязательна для корректной
  интерпретации. Ограничения размера и rate limit — защитная policy Knossos, не норма VRWeb.
...
1. `ReplicatedStateStore`: registry схем, Subject Bindings, составные access rules, атомарный
   reducer-result `{state, bindings}`, revision, delta, snapshot, epoch/seq/gap и wire limits.
...
Настоящий сетевой split-brain с последующим слиянием двух одновременно живых компонент mesh
пока не покрыт fault-injection тестом. Оценка часов authority для видео понадобится, только если
практически проявится заметный дрейф. Обе работы учитываются в
[едином roadmap](../roadmap.md#p1--надёжность-replicated-state).

## Критерий решения после пилота

Продолжать, если generic-слой берёт late join, bindings, validation и resync без веток
