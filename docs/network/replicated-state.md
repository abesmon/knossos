...
> конфликт команд, смену authority и reconnect с восстановлением snapshot.
>
...
1. E2E `tests/run_net_replicated_state_test.py` автоматически проверяет late join, команды
   ...
   play/pause/seek с обоих клиентов также проверены вручную. Не покрыт только настоящий
   сетевой split-brain с последующим слиянием двух одновременно живых компонент mesh.
2. Метрики byte/event rate и ограничение/чанкование суммарного snapshot; сейчас ограничены
   число объектов, поля и отдельные значения, но не итоговый сериализованный размер снимка.
3. Если дрейф заметен — оценка часов authority и вычисление позиции от временного якоря.
