...
- [realtime-resources.md](realtime-resources.md) — realtime-ресурсы (блобы): бинарные данные,
  создаваемые в сессии (картинки инструмента размещения), контент-адресация
  `vrwebblob://sha256/…`, p2p-передача и документная форма для флаша.
- [object-id.md](object-id.md) — формат `id` объекта (`<user8>.<seq>`): структура, зачем так,
  почему префикс недоверенный. Общий для эфемерного слоя и флаша.
