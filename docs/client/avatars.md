...
- **Видимое тело себя (для зеркал).** [`LocalAvatar`](../../actors/avatar/local_avatar.gd) —
  ...
  на слое видимости `LocalAvatar.AVATAR_LAYER`: камера первого лица и камера загрузочного
  хаба исключают его через `cull_mask`, а камеры зеркал — рендерят. Так игрок видит себя
  только в отражении, но не из первого лица и не в нейтральном пространстве между мирами,
  как в VRChat. См. [устройство зеркала VRWML](scene-elements.md#зеркало-vrwml).
- **UI выбора аватара.** Экран настроек ([scenes/settings.gd](../../scenes/settings.gd)) имеет
  ...
  пока нет; он учитывается в [едином roadmap](../roadmap.md#p2--performance-streaming-и-ui).
