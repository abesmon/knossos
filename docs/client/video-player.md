...
- Провести двухклиентский soak-тест Replicated State: одновременный seek, late join, смена
  authority и reconnect. Обычные play/pause/seek с обоих клиентов уже проверены вручную.
  Решение и границы — в [replicated-state.md](../network/replicated-state.md).
