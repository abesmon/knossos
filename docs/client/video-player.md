...
- луч ушёл с экрана (`hover_at` не звали дольше `UI_LOST_HIDE`), **или**
- точка касания не двигалась дольше `UI_IDLE_HIDE` (мышь «замерла»).

Механика: игрок каждый физ-кадр кормит объект под прицелом методом `hover_at(point)` (общий
канал непрерывного наведения, в отличие от `interact_at` по клику — см.
`Player._dispatch_hover` в [actors/player/player.gd](../../actors/player/player.gd)). Экран сам ведёт
таймауты и затухание (`_update_ui`), поэтому явный «hover-exit» не нужен — достаточно перестать
звать `hover_at`. Сдвиг точки меньше `UI_MOVE_EPS` считается «мышь не двигалась».
...
> Ручное следование за редиректами через `HTTPClient` — возможное улучшение на будущее.
...
- Настоящий стриминг (HLS/DASH) поверх прогрессивной докачки, что уже есть.
- Передача роли контроллера и опциональный режим owner-презентера.
- UI-контролы: полоса прогресса и буфер на экране **есть** (см. «Наэкранный UI»); дальше —
  интерактив (seek кликом по бару, регулятор громкости).
