...
## Архитектура: базовый уровень и надстройки

Плеерная система разложена на два уровня (движемся к модели VRChat, но со стандартной
синхронизацией «из коробки»):

1. **Базовый уровень — панель + источник, никакой сети.** `VrwebVideoPlayer` (декод в
   текстуру, локальный транспорт play/pause/seek, смена источника `set_source`) и
   `VrwebVideoScreen` (поверхность). Сами по себе они ничего не знают о синхронизации.
2. **Надстройки над базовым уровнем:**
   - **Стандартная синхронизация (по умолчанию).** `VrwebVideoManager` регистрирует плеер в
     Replicated State и гоняет heartbeat таймкипера (см. «Синхронизация» ниже). Именно в эту
     надстройку превращается HTML `<video>` — стандартный сценарий «все смотрят вместе»
     работает без единой строчки кода автора.
   - **Ручная синхронизация скриптингом.** Тег с `sync="none"` отключает стандартную
     надстройку: менеджер лишь привязывает экраны, а транспортом и источником управляет
     Luau-скрипт страницы (capability `vrweb/video/1`, см.
     [scripting-api.md](../space/scripting-api.md#видео-плееры-vrwebvideo1)). Автор мира сам
     решает, что и как синхронизировать: поле ввода ссылок, караоке с компенсацией задержки
     голоса, управление по рангам — любые правила поверх `document.state`/`document.remote`.

Атрибут `sync` принимает `none`/`off`/`false`/`manual` (= выключить) на обоих тегах; у
`<VRWebVideoScreen src="...">` он относится к неявному плееру. Отсутствие атрибута — плеер
синхронизируется стандартно. Скрипт может управлять и synced-плеером: его play/pause/seek
пройдут тот же путь authority-валидации, что клик игрока по экрану.

**Граница надстройки — по построению.** Стандартная синхронизация реплицирует только
транспорт (play/pause/позиция) и никогда — `src`. Это не временное ограничение, а следствие
устройства уровня: стандартный компонент синтезируется из декларативного тега (`<video>`,
VRWML), у которого источник задан разметкой и одинаков у всех клиентов сам по себе; никакой
...
| `sync` | `none`/`off`/`false`/`manual` — отключить стандартную сетевую синхронизацию (базовый плеер под ручной скриптинг) |
...
| `src` | ИЛИ свой источник — тогда создаётся **неявный** плеер (ключ по URL: одинаковый `src` = общий плеер). Поддерживает `autoplay`/`loop`/`volume`/`sync` |
...
Это **стандартная надстройка** над базовым плеером (см. «Архитектура» выше): включена по
умолчанию для всех плееров без `sync="none"`, включая HTML `<video>`. Плееры с `sync="none"`
менеджер в Replicated State не регистрирует и heartbeat по ним не шлёт.
...
| Файл | Роль |
|---|---|
| [scripts/vrweb_video_player.gd](../../scripts/vrweb_video_player.gd) | логический плеер (базовый уровень): FFmpeg-декод в текстуру, прогрессивная докачка src (буферизация, ранний старт), транспорт, смена источника `set_source` на лету, флаг `synced`, сигналы `finished`/`playback_error` + `last_error()` (download_failed/decoder_unavailable/decode_failed) |
...
текущую позицию каждый клиент выводит сам из `document.clock.authority_time()`, а
дрифт-коррекция (порог, кулдаун) — локальный код мира. Клики по экрану и кнопки пульта
публикуются в канон через подписку на `transport_changed`; поздно вошедшие получают всё из
...
> в [едином roadmap](../roadmap.md#p0--scripting-transport-boundaries).
