...
- **Текстовый чат** — поверх WebRTC data-канала. Лимит сообщения — 280 символов
  ...
  «просыпается» при новом сообщении или возврате мыши. В UI-режиме текст лога можно
  выделять мышью и копировать (Ctrl/Cmd+C — лог получает фокус по клику, `selection_enabled`).
  Ссылки в сообщениях кликабельны: `_linkify` оборачивает найденные URL (http(s), `www.`,
  `mailto:`/`tel:`/прочие схемы, deeplink `scheme://…`) в `[url]`, а `_on_chat_meta_clicked`
  гонит адрес через `TopologyBuilder.classify_href` — ту же классификацию, что у ссылок
  страницы: навигация VRWeb, телепорт к якорю `#…` или внешнее намерение `OS.shell_open`.
...
**Это отдельная ось от облика мира.** Кто с кем встречается (инстанс) — по полному URL,
«важно всё». А как мир ВЫГЛЯДИТ (сид геометрии) — по `host + подпись топологии`
(`PageFetcher.space_seed`, см. [geometry-lab.md](geometry-lab.md)): две топологически
одинаковые страницы одного сайта — разные комнаты, но идентичное пространство.
...
**Эфемерные изменения сцены.** Поверх авторитета — транспортный слой объектов, добавленных в
общий мир на лету (не из исходного HTML). Модель **команда/событие**: инициатор шлёт авторитету
ДЕЙСТВИЕ (add/update/remove/reparent — только нужную мутацию), авторитет валидирует против своего
состояния и прав и рассылает упорядоченные СОБЫТИЯ `(epoch,seq)`; новички/смена авторитета —
снимком. Контракт движок-агностичен (чистая машина `SceneChanges`), правки — только своих объектов
(фундамент под систему прав), структура JSON-сериализуема (готова к выгрузке на сервер). Первый
инструмент — **пузыри** (временные порталы «ушёл сюда», 30 c). Полностью — в
[ephemeral-changes.md](ephemeral-changes.md).
...
Список ICE-серверов (STUN + TURN-relay с учётками) задаётся **не в коде**, а в приватном
конфиге сборки `config/build.private.cfg` и читается автолоадом `BuildConfig`
(`NetworkManager` берёт его как `BuildConfig.ice_servers`). Адреса и логин/пароль TURN —
...
База федерации — **домашние серверы** ([home-server.md](home-server.md)): адреса
`nick@domain`, discovery через `/.well-known/vrweb`, сертификаты идентичности. Монолит
домашнего сервера включает сигналинг тем же протоколом (эндпоинт `/signal`). Клиент уже
интегрирован: логин и сертификаты — autoload `HomeServer` (вкладка «Аккаунт» настроек),
пиры с сертификатом проверяются в меше (`NetworkManager.identity_verified`), а адрес
сигналинга резолвится по приоритету «настройка пользователя → анонс домашнего сервера →
дефолт сборки» (`Settings.effective_signaling_url`).
...
3. Запустить два экземпляра клиента. В обоих: настройки → включить «Онлайн», адрес
   ...
   кнопкой «✕»/«Сбросить» — тогда подставятся дефолты: адрес сигналинга из
   `BuildConfig.signaling_url` (приватный конфиг сборки, см. [build-config.md](build-config.md);
   схема приводится к `wss://`), ник `Guest-XXXX`, лицо `resources/default_face.png`.
