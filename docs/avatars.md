...
> [home-server.md](home-server.md)). Раньше вместо `warning`/`err` рисовался текстовый «⚠».

> **Зависимость от слоя идентичности.** Вся стойкость = стойкость проверки «а правда ли пир —
> это `user@host`». Этого слоя ([Слой 3](vrweb-overview.md)) ещё нет: `evaluate` зовётся с
