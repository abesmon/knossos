...
|---|---|---|
| `ALLOWED` | `allow` содержит `*`, **или** личность верифицирована и совпала с паттерном | показываем, без предупреждения |
| `UNCONFIRMED` | манифеста нет/не достать, **или** личность пира пока невозможно верифицировать | показываем + жёлтый «⚠» у ника |
| `DENIED` | манифест есть, личность верифицирована, но носителя в `allow` нет | (цель) скрыть кастомный аватар → дефолтная капсула |
...
> `identity_verified = false`, поэтому сейчас всё кроме `*` даёт `UNCONFIRMED` (жёлтый «⚠»), а
...
- **В любом случае** при не-`ALLOWED` рисуем у ника жёлтый «⚠» («законность не подтверждена»).
