...
скриптов), `3ef7f50` (public scripting API и package-demo) и `2fcc80a` (единая content
policy для document/live-peer путей).
...
- [x] Применять одну client-side policy к документу и `vrweb-node`/`vrweb-patch` от пиров.
  Persistent content после flush снова загружается как document и проходит ту же проверку.
- [ ] Повторить существенные content-policy правила на серверной границе persistence flush,
  чтобы опасный контент отклонялся до записи, а не только при следующей загрузке клиентом.
...
1. Вывести audit `VrwebContentPolicy` в debug UI и вынести class/property/resource rules в
   data-driven registry.
2. Добавить первый opt-in safe profile: блокировка очевидно опасных properties/classes и
   бюджеты дерева/ресурсов; одинаковые тесты для document и live-peer путей.
3. Включить module identity в multiplayer compatibility и проверить mismatch двумя чистыми
   клиентами.
4. Закрыть navigation generation/redirect policy и добавить hostile/negative fixtures до E2E
   trusted modules MVP.
