...
Content cache и распакованный `module_root` проходят через один `Sandbox.resolve` namespace.
Это обязательно для параллельных клиентов: registry загружает script ровно из каталога, куда
unpacker записал package, а не из общего несandboxed `user://`.
