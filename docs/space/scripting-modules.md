...
- повторный локальный экспорт уже даёт byte-identical ZIP; осталось проверить deterministic
  output и bundled imported assets по format/platform matrix, добавить сложные import options;
...
Пользовательский self-contained package-demo уже добавлен в `test_pages/index.html`: рядом с
готовым `.vrmod` лежат fixture и воспроизводимый `tests/build_package_demo.gd` для пересборки.
