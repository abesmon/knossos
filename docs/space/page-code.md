# Пользовательские классы и скрипты со страницы
...
func mount(context):
    context.log("light switch mounted")
</script>
...
- Задать лимиты: compressed/unpacked bytes, files, scripts, components, source length и время
  компиляции. Значения держать в одном `PageModuleLimits`, а не размазывать по loader'ам.
...
Начат в [tests/test_page_script_runtime.gd](../../tests/test_page_script_runtime.gd): probe
...
Базовая реализация: [page_module_collector.gd](../../scripts/page_modules/page_module_collector.gd)
собирает inline/src/package IR; [page_module_manifest.gd](../../scripts/page_modules/page_module_manifest.gd)
валидирует manifest format 1; [page_module_integrity.gd](../../scripts/page_modules/page_module_integrity.gd)
реализует принятую same-/cross-origin policy.

Добавить независимый от UI слой примерно следующей формы:

```text
PageModuleDefinition  — id/runtime/hash/exports/permissions/files
PageModuleCollector   — HTML <script>/<VRWebModule> -> definitions
PageModuleValidator   — manifest, пути, MIME, лимиты, integrity
PageModuleRegistry    — module id -> загруженная версия/exports текущей навигации
```
...
[page_module_cache.gd](../../scripts/page_modules/page_module_cache.gd): артефакты дедуплицируются
...
[page_module_fetcher.gd](../../scripts/page_modules/page_module_fetcher.gd). Fetcher намеренно
отклоняет redirects до появления проверки каждого нового origin. `.vrmod` проверяется и
извлекается [page_module_package.gd](../../scripts/page_modules/page_module_package.gd): лимиты,
...
### Этап 4. Trust policy и UX

**Отложенный крупный feature milestone.** На текущем этапе действует явно задокументированный
prototype `ALLOW_ALL`; этот этап нельзя считать выполненным и нельзя выпускать production build
с пользовательским кодом без него.

- Добавить глобальный режим `allow_all|selected|deny_all` и persistent store exact hashes,
  module identities, origin rules и provenance записи (local/home-server/trust-list).
- `allow once` живёт только до конца процесса либо загрузки — границу зафиксировать в UX.
- Перед компиляцией показать origin, module id, hash, runtime и заявленные permissions.
- Один preflight review агрегирует модули страницы, не создавая prompt storm; compile/materialize
  начинаются только после решений по всем blocking entries.
- Добавить экран списка доверенных origin с отзывом, временем решения и последними hashes.
- Без UI (headless/tests) policy по умолчанию deny; тестовый флаг разрешения должен быть явным.
...
**Готово, когда:** неизвестный origin не исполняется до ответа, deny запоминается, allow once не
переживает выбранную границу, revoke прекращает следующий запуск.

### Этап 5. Trusted GDScript runtime и lifecycle

Первый inline-срез реализован в
[page_module_registry.gd](../../scripts/page_modules/page_module_registry.gd): registry принимает
...
definitions, применяет временный prototype `ALLOW_ALL` и передаёт registry в Builder.
Inline, single-file `src` и script exports `.vrmod` подключены к navigation pipeline.
Минимальный lifecycle реализован через
[page_module_context.gd](../../scripts/page_modules/page_module_context.gd): `mount(context)`
после `_ready`, однократный `unmount()` при выходе из дерева и инвалидируемые `scene_root` /
`lifecycle/1`. [page_module_session.gd](../../scripts/page_modules/page_module_session.gd)
...
[page_module_state_api.gd](../../scripts/page_modules/page_module_state_api.gd): namespaced
schema/object, command/read/revision/subscriptions и автоматическая очистка. `context.timers`
реализован в [page_module_timer_api.gd](../../scripts/page_modules/page_module_timer_api.gd):
...
[page_module_asset_api.gd](../../scripts/page_modules/page_module_asset_api.gd): он открывает
...
сложные import options ещё не реализованы. Trust persistence и остальные facade
`KnossosPageAPI` также ещё не реализованы.
...
Реализованы `lifecycle/1`, `scene-root/1` и `replicated-state/1`; state facade принадлежит
module-level session, поэтому несколько components не снимают схемы друг у друга.

Сделать отдельные facade-объекты, выдаваемые через context, а не ссылку на `main`/autoload:

- `scene` — root компонента и разрешённые операции;
- `state` — register/read/command/unregister Replicated State;
- `input` — world interaction hooks без зависимости от `Player`;
...
- `log` и lifecycle-safe timers; первая версия `context.timers.start/cancel/cancel_all` готова;
- `features.has("name/version")`.
