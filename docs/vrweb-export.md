...
| Открыть `.html`/`.htm` из FileSystem | открывает `<vrweb>` как editable scene, а HTML вокруг — как видимый read-only procedural preview |
| **Сохранить импортированный HTML** / Save | заменяет только исходный `<vrweb>…</vrweb>`, не нормализуя остальной HTML |
...
Без opt-in `strict` export останавливается; `compatible` сохраняет геометрию/свойства без
поведения и выводит warning.
...
Для выбранного scripted node панель также редактирует `module id`, SemVer `version`,
`permissions`, `requires` и `optional`. Эти значения принадлежат portable addon, сохраняются
как metadata сцены и для package попадают в `vrweb-module.json`. `permissions` — декларация для
trust-review, а не sandbox. Панель явно напоминает, что оба режима исполняют trusted GDScript;
inline ограничен одним файлом, package включает manifest и зависимости.

`inline` допустим только для самодостаточного GDScript без module-local файлов. Если exporter
видит явные scene/asset/script dependencies, он предлагает `package`. `@tool`, `class_name`,
`load`/`preload`, script inheritance, абсолютные resource paths и literal `</script>` для
inline отклоняются; C#/GDExtension не являются source GDScript. Неявную ссылку на autoload
статический validator надёжно определить не может. Полный действующий контракт —
[scripting-modules.md](space/scripting-modules.md).

Inline source подготавливает portable `VrwebInlineExporter`: он не зависит от builder-а
Knossos, запрещает `load`/`preload`, script inheritance, абсолютные resource paths, `@tool`,
`class_name` и literal `</script>`. HTML всегда получает явный
`data-mode="trusted-gdscript"`; default runtime больше не остаётся неявным.

Inline round-trip (script, `@export`-свойство, базовое свойство и дети) проверяет
`tests/test_inline_export.tscn`. Package round-trip проверяет
...
Структурированный export report содержит `ok/errors/warnings`, profile/catalog,
package id/version/permissions/requires/optional, file/hash/integrity, списки файлов и assets.
Editor показывает итоговый review и не
записывает HTML при strict/package-ошибке. Повторная сборка
одинаковых входов проверяется на byte-identical `.vrmod`.

Автоматическая проверка сложных import options и всех форматов на каждой целевой платформе
не входит в текущее покрытие.

Package exporter отклоняет `@tool`, `class_name`, абсолютные `res://`-ссылки,
выход пути за каталог проекта и непереносимые ссылки. Это проверка переносимости
...
| `addons/vrweb_tools/vrweb_compatibility.gd` | Versioned strict vocabulary и budgets, общий источник editor/CLI diagnostics |
...
| `addons/vrweb_tools/vrweb_markup_materializer.gd` | Strict declarative `<vrweb>` → editable Godot subtree без runtime actors |
...
| `integrations/knossos/vrweb_tools/vrweb_html_scene_codec.gd` | Knossos adapter: добавляет procedural preview вокруг portable imported `<vrweb>` |
...
| `addons/vrweb_tools/vrweb_html_document.gd` | Lossless-поиск диапазона первого настоящего `<vrweb>` вне comment/raw-text |
...
- дети `<vrweb>` материализуются обычными редактируемыми узлами;
- в `combine` DOM за пределами `<vrweb>` проходит полный production pipeline
...
- в `exclusive` HTML DOM вообще не строится — во viewport остаётся только `<vrweb>`;
...
- runtime scripted actors (`RichPanel`, video, mirror, component и неизвестные public classes)
  не запускаются как `@tool`. Если они встречены внутри `<vrweb>`, весь декларативный слой
...
только обычных детей корня в новый `<vrweb>` и заменяют исходный диапазон блока. Scanner
пропускает HTML comments и содержимое `script/style/textarea/title`, поэтому текст
`"<vrweb>"` внутри JS не принимается за сцену. Prefix и suffix сохраняются буквально, включая
пробелы и CRLF. Хэш исходного блока запоминается при импорте; если сам `<vrweb>` параллельно
...
Актуальное состояние Maker Kit и оставшиеся release-проверки описаны в
[общем roadmap](roadmap.md#vrweb-maker-kit--актуальное-состояние). Format constants,
public-class registry и package integrity находятся в переносимом addon.
