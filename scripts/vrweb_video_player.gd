...
##
## Два уровня (см. docs/client/video-player.md): сам плеер — БАЗОВЫЙ уровень (панель +
## источник, никакой сети). Стандартная синхронизация — надстройка менеджера, включена по
## умолчанию и отключается атрибутом sync="none" (synced == false): такой плеер менеджер
## только привязывает к экранам, а транспортом управляет скриптинг страницы (vrweb/video/1).
...
## Реальный конец ролика: файл целиком на диске, намерение играть, loop выключен.
## Ложный EOF докачки сюда не попадает (его разруливает машина состояний _process).
signal finished
## Терминальная ошибка воспроизведения (код — как в last_error()). Ошибки, случившиеся до
## подписки (например, нет FFmpeg-аддона на _ready), доступны опросом last_error().
signal playback_error(code: String)

var id: String = ""
## Участвует ли плеер в стандартной сетевой синхронизации (надстройка VrwebVideoManager).
## false (тег с sync="none") — чисто локальный базовый плеер под ручной скриптинг.
var synced := true
...
var _pipeline_gen := 0           # поколение пайплайна: set_source инвалидирует колбэки старой докачки
var _last_error := ""            # "" | download_failed | decoder_unavailable | decode_failed
...
func _ready() -> void:
	if not is_available():
		if _src_url != "":
			...
			# Эмитить сигнал бессмысленно: подписчики (экраны, скрипты) появляются позже.
			_last_error = "decoder_unavailable"
		return
	_want_playing = _autoplay
	# Плеер без src — валидный базовый уровень: источник назначит скриптинг (set_source).
	if _src_url == "":
		return
	_ensure_vsp()
...
## Создаёт скрытый VideoStreamPlayer при первом использовании (в _ready с src из тега или
## позже — при первом set_source из скриптинга).
func _ensure_vsp() -> void:
	if _vsp != null:
		return
	...
	_vsp.visible = false
	add_child(_vsp)
	_vsp.finished.connect(_on_vsp_finished)


## finished штатного VSP: во время докачки это ложный EOF (его разруливает машина состояний
## _process), при включённом loop VSP перезапускается сам. Реальный конец — только когда файл
## целиком на диске и играть намеревались.
func _on_vsp_finished() -> void:
	if not _download_done or _loop or not _want_playing:
		return
	_vlog("конец ролика")
	finished.emit()


## Терминальная ошибка воспроизведения: фиксируем для опроса и оповещаем подписчиков.
func _fail(code: String) -> void:
	_last_error = code
	_vlog("ошибка воспроизведения: " + code)
	playback_error.emit(code)


## Последняя терминальная ошибка ("" — ошибок нет): download_failed (докачка сорвалась до
## первого кадра), decoder_unavailable (нет FFmpeg-аддона), decode_failed (файл целиком на
## диске, но декодер не дал ни кадра — битый файл/не видео).
func last_error() -> String:
	return _last_error


## Текущий URL источника (для скриптинга).
func source() -> String:
	return _src_url


## Смена источника на лету (базовый уровень, зовёт скриптинг vrweb/video/1): сбрасывает
## пайплайн докачки/декода и запускает его заново с новым URL. Намерение play/pause
## сохраняется: играл — новый источник начнёт играть, стоял на паузе — покажет постер.
## Стандартная надстройка синхронизации src НЕ реплицирует — менять источник synced-плеера
## должен одинаковый скрипт на всех клиентах (или кастомный state автора мира).
func set_source(url: String) -> bool:
	...
	_reset_pipeline()
	if not is_available():
		_fail("decoder_unavailable")
		return false
	_ensure_vsp()
	_vlog("set_source: %s" % clean)
	_resolve_source()
	return true


## Останавливает текущую докачку/декод и обнуляет машину состояний. Поколение _pipeline_gen
## растёт: колбэк завершения старого HTTPRequest (мог уже лететь) отбрасывается по нему.
func _reset_pipeline() -> void:
	...
	if _vsp != null:
		...
		_vsp.stream = null
		if "loop" in _vsp:
			_vsp.loop = false   # включится снова после полной загрузки (_maybe_enable_loop)


## Громкость 0..1 (для скриптинга; звук идёт через шину World).
...
func _download(url: String, part_path: String, final_path: String) -> void:
	...
	add_child(_http)
	var gen := _pipeline_gen   # set_source сбрасывает пайплайн — колбэк старой докачки игнорируем
	_http.request_completed.connect(
		func(result, code, _headers, _body):
			if gen == _pipeline_gen:
...
func _process(delta: float) -> void:
	...
	if _opened and _texture == null:
		...
		elif not _ever_started and _download_done and Time.get_ticks_msec() > _probe_deadline_ms:
			# файл целиком на диске, а кадра так и нет — битый файл или не видео. Терминально.
			_vsp.stream = null
			_opened = false
			_fail("decode_failed")
			set_process(false)
