...
func _on_ui_accept(uv: Vector2) -> void:
	if _player == null:
		return
	if _ui_alpha > 0.5 and _seek_at_uv(uv):
		return
	_player.toggle()
...
func _seek_at_uv(uv: Vector2) -> bool:
	var dur: float = _player.duration()
	if dur <= 0.0:
		return false
	var local := Vector2((uv.x - 0.5) * _quad.size.x, (0.5 - uv.y) * _quad.size.y)
...
func _on_ui_pointer_move(uv: Vector2) -> void:
	_since_hover = 0.0
	var move_m := Vector2((uv.x - _last_hover.x) * _quad.size.x, (uv.y - _last_hover.y) * _quad.size.y)
	if not _have_hover or move_m.length() > UI_MOVE_EPS:
		_idle_accum = 0.0
	_last_hover = uv
	_have_hover = true
