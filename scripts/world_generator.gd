...
func _place_cluster(conn: int, desired: Dictionary, blocked: Dictionary) -> void:
	...
	for op in layout["openings"]:
		...
		_add_opening(op["b"], (op["bcell"] as Vector2i) + off, op["bdir"])
...
func _add_wall_seg(holder: Node3D, w: Dictionary, lo: float, hi: float, off_half: float, h: float, color: Color) -> void:
	...
	var length := hi - lo
	if w["kind"] == "x":
		_add_box(holder, Vector3(WALL_THICK, h, length), Vector3(w["sign"] * off_half, h * 0.5, center), color, true)
	else:
		_add_box(holder, Vector3(length, h, WALL_THICK), Vector3(center, h * 0.5, w["sign"] * off_half), color, true)
