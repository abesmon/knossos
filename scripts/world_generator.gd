...
# Спавн по внутреннему маршруту «верхней» комнаты страницы. Верх = минимальный hints.doc_order
# среди листьев со своим наполнением (обычно хэдер/титул) — а не root-комната <body>, где часто
# нет полезного контента. Первый объект страницы остаётся fallback'ом, если раскладка не дала routes.
var _spawn_target_id: int = -1     # комната, в которой спавним (см. _find_spawn_room_id)
...
var _spawn_room_id: int = -1       # комната первого объекта: по её реальному футпринту валидируем спавн
...
var _spawn_done: bool = false         # спавн уже задан напрямую (root-route или титульный проём)
...
func _build(space: Dictionary, parent: Node3D, seed_value: int, on_transition: Callable) -> void:
	...
	# Синхронно (до первого кадра): свет/небо — чтобы мир был освещён, и root-комната —
	# чтобы spawn_point по её route был готов к возврату из generate() и игроку было куда встать.
	...
	_build_room(_root_id, _container, on_transition)
	_built[_root_id] = true
	# «Верхняя» комната (хэдер/титул) — цель спавна. Её тоже строим синхронно, чтобы у игрока
	# сразу был пол/стены под ногами, а не пустота до подхода стриминга.
	_spawn_target_id = _find_spawn_room_id()
	if _spawn_target_id != -1 and not _built.has(_spawn_target_id):
		_build_room(_spawn_target_id, _container, on_transition)
		_built[_spawn_target_id] = true
	_compute_spawn()
...
## «Верхняя» комната страницы: минимальный hints.doc_order (порядок чтения) среди листьев со
## своим наполнением. Лист = комната без подкомнат: спавним в самом контенте, а не в развилке.
## Наполнение обязательно — коридоры/коннекторы без объектов как точка спавна бессмысленны.
## Так игрок оказывается в хэдере/титуле, а не в пустой root-комнате <body>. Фолбэк — root.
func _find_spawn_room_id() -> int:
	var best_id: int = -1
	var best_order: int = 0x7FFFFFFF
	for id in _rooms.keys():
		var room: Dictionary = _rooms[id]
		if (room["objects"] as Array).is_empty():
			continue                       # без наполнения — коридор/коннектор, спавнить незачем
		if not (room["children"] as Array).is_empty():
			continue                       # не лист — контент лежит в подкомнатах
		var order: int = int(room["hints"].get("doc_order", 0x7FFFFFFF))
		if order < best_order:
			best_order = order
			best_id = id
	return best_id if best_id != -1 else _root_id


## Основной спавн — первая route-клетка «верхней» комнаты (_spawn_target_id). Первый объект —
## fallback: встаём от объекта внутрь комнаты и проецируем точку в реальный футпринт.
func _compute_spawn() -> void:
	if _spawn_done:
		return
	var target: int = _spawn_target_id if _spawn_target_id != -1 else _root_id
	if _compute_room_route_spawn(target):
		return
	if _has_spawn_obj:
		var d: float = clampf(3.5, 1.2, _spawn_max_d)
		spawn_point = _valid_spawn_floor(_spawn_room_id, _spawn_obj_world, _spawn_inward, d) + Vector3(0, 1.0, 0)
		spawn_look_at = _spawn_obj_world + Vector3(0, 1.6, 0)
		has_spawn_look = true
		return
	spawn_point = _room_spawn_fallback_floor(target) + Vector3(0, 1.0, 0)


func _compute_room_route_spawn(room_id: int) -> bool:
	var rd: Dictionary = _layout.get("rooms", {}).get(room_id, {})
	for route in rd.get("routes", []):
		if route.is_empty():
			continue
		var cell: Vector2i = route[0]
		if not _foot.get(room_id, {}).has(cell):
			continue
		spawn_point = _cell_world(cell.x + 0.5, cell.y + 0.5) + Vector3(0, 1.0, 0)
		for i in range(1, route.size()):
			var next: Vector2i = route[i]
			if next == cell:
				continue
			if not _foot.get(room_id, {}).has(next):
				continue
			spawn_look_at = _cell_world(next.x + 0.5, next.y + 0.5) + Vector3(0, 1.6, 0)
			has_spawn_look = true
			break
		_spawn_done = true
		return true
...
func _world_cell(p: Vector3) -> Vector2i:
	return Vector2i(int(floor((p.x - _shift.x) / GRID)), int(floor((p.z - _shift.z) / GRID)))


func _point_in_room_floor(room_id: int, p: Vector3) -> bool:
	return _foot.get(room_id, {}).has(_world_cell(p))


## Возвращает точку на полу внутри реального клеточного футпринта комнаты. Для сложных
## L/U-форм прямой отступ от стены может попасть в пустую клетку bbox, поэтому сначала
## отступаем по лучу назад, а если луч целиком плохой — берём ближайший центр клетки комнаты.
func _valid_spawn_floor(room_id: int, origin: Vector3, inward: Vector3, distance: float) -> Vector3:
	var desired := origin + inward * distance
	if _point_in_room_floor(room_id, desired):
		return desired
	var d := distance - 0.25
	while d >= 0.75:
		var p := origin + inward * d
		if _point_in_room_floor(room_id, p):
			return p
	return _nearest_room_floor_point(room_id, desired)


func _room_spawn_fallback_floor(room_id: int) -> Vector3:
	var desired: Vector3 = _positions.get(room_id, Vector3.ZERO)
	if _point_in_room_floor(room_id, desired):
		return desired
	return _nearest_room_floor_point(room_id, desired)


func _nearest_room_floor_point(room_id: int, desired: Vector3) -> Vector3:
	var best := desired
	var best_d := INF
	for c in _foot_cells.get(room_id, []):
		var p := _cell_world(c.x + 0.5, c.y + 0.5)
		var dx := p.x - desired.x
		var dz := p.z - desired.z
		var dd := dx * dx + dz * dz
		if dd < best_d:
			best_d = dd
			best = p
...
func _record_spawn(room_id: int, world_pos: Vector3, face: Vector2i) -> void:
...
func _build_room_title(holder: Node3D, id: int, title_obj: Dictionary) -> bool:
	...
	if title_obj["id"] == _first_obj_id:
		_record_title_spawn(id, pos, -out_n, GRID * 1.2)
	return true
...
func _record_title_spawn(room_id: int, door_world: Vector3, inward: Vector3, reach: float) -> void:
	if _spawn_done:
		return
	var d: float = clampf(reach * 0.6, 1.2, 3.0)
	spawn_point = _valid_spawn_floor(room_id, door_world, inward, d) + Vector3(0, 1.0, 0)
...
func _reposition_room_objects(room_id: int) -> void:
	...
	for obj in _rooms[room_id].get("objects", []):
		...
		if obj_id == _first_obj_id:
			_record_spawn(room_id, Vector3(node.position.x, 0.0, node.position.z), face)
