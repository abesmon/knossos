...
# Спавн по внутреннему маршруту root-комнаты. Первый объект страницы остаётся fallback'ом,
# если раскладка не дала routes.
...
## Основной спавн — первая route-клетка root-комнаты. Первый объект используется только как
## fallback: встаём от объекта внутрь комнаты и проецируем точку в реальный футпринт.
func _compute_spawn() -> void:
	if _spawn_done:
		return
	if _compute_root_route_spawn():
		return
	if _has_spawn_obj:
		...
		has_spawn_look = true
		return
	spawn_point = _room_spawn_fallback_floor(_root_id) + Vector3(0, 1.0, 0)


func _compute_root_route_spawn() -> bool:
	var rd: Dictionary = _layout.get("rooms", {}).get(_root_id, {})
	for route in rd.get("routes", []):
		if route.is_empty():
			continue
		var cell: Vector2i = route[0]
		if not _foot.get(_root_id, {}).has(cell):
			continue
		...
		for i in range(1, route.size()):
			...
			if next == cell:
				continue
			if not _foot.get(_root_id, {}).has(next):
				continue
