...
const DOOR_HEIGHT := 2.4        # высота проёма; над ним остаётся перемычка-стена (опора вывески-заголовка)
const WALL_THICK := 0.15        # тонкая стена, сдвинутая внутрь комнаты на полтолщины (см. _add_edge_wall): у вплотную стоящих комнат стены стыкуются грань-в-грань, без z-fight
const ROUTE_EXPAND := 16        # запас области поиска коридора вокруг пары комнат, клеток

# --- Расстановка объектов по стенам ---
const OBJECT_INSET := 0.35      # отступ объекта от своей стены внутрь комнаты, м (панель висит НА стене, а не в воздухе перед ней)
...
# --- ПРОТОТИП v1: органическая форма комнаты (фаза 2.5) ---
# Прямоугольный футпринт комнаты режется в полимино (набор клеток) — комнаты перестают быть
# квадратными. Стены обходят контур полимино (осевые, лесенкой по срезам), пол кладётся
# поклеточно. Режутся только УГЛЫ — середины всех четырёх сторон остаются полом, поэтому
# двери/коридоры/вывески-заголовки/настенные объекты по-прежнему садятся на периметр bbox.
# Не режем: коннекторы и комнаты-кластера (склеены вплотную к коннектору — им нужна прямая стена).
const SHAPE_ENABLE := true        # включить нерегулярные формы (false — квадраты как раньше)
const SHAPE_FORCE := ""           # "" = архетип коннектора по сиду; иначе: квадрат/фаска/октагон
const CONNECTOR_ASPECT_MAX := 2.2 # макс. отношение сторон прямоугольника коннектора
# Комнаты: по умолчанию прямоугольники; лишь часть отклоняется (и то компактно, без «кишок»).
const IRREGULAR_CHANCE := 0.6     # доля комнат, отклоняющихся от прямоугольника (0 — все прямоугольные)
const ROOM_ASPECT_MAX := 1.7      # макс. вытянутость прямоугольной комнаты
const BIG_AREA_CELLS := 30        # комнаты ≥ этой площади остаются «архитектурными» (rect/L/ниша; без креста/пентамино)
const PENTO_TRIES := 6            # попыток размещения куска при КОМПАКТНОЙ пентамино-сборке (тянем к центру)
const ROOM_SHAPE_FORCE := ""      # "" = по сиду; иначе форсить форму комнаты: прямоугольник/скос/угол/ниша/крест/пентамино
...
var _room_cells: Dictionary = {}  # roomId -> Vector2i: bbox футпринта в клетках (ширина, глубина)
var _room_shape: Dictionary = {}  # roomId -> { Vector2i(локальная клетка): true } — клетки пола (полимино внутри bbox)
...
var _bbox_cells: Dictionary = {}  # Vector2i -> true: ВСЕ клетки bbox комнат (вкл. срезанные) — коридоры их не пересекают
...
func _build(space: Dictionary, parent: Node3D, seed_value: int, on_transition: Callable) -> void:
	...
	_compute_room_shapes()                   # фаза 2.5 (прототип): режем bbox в полимино
	_finalize_positions()
	_fill_occupancy()
...
func _compute_room_dim(id: int) -> void:
	...
	# Форма футпринта (объекты теперь стоят по ВСЕМУ контуру стен, см. _wall_spans):
	#   • выключено  -> квадрат cw×cw (как было);
	#   • коннектор  -> неравносторонний прямоугольник той же площади (срез углов — фаза 2.5);
	#   • комната    -> нерегулярный полимино из пентамино под ту же площадь (cw² клеток).
	if not SHAPE_ENABLE:
		_set_room_box(id, Vector2i(cw, cw))
	elif _rooms[id]["kind"] == "connector":
		_set_room_box(id, _rect_dims(cw, id, CONNECTOR_ASPECT_MAX))
	else:
		var shape := _make_room_shape(cw, id)
		var bb := _bbox_of(shape)
		_room_cells[id] = bb
		_room_size[id] = Vector2(bb) * GRID
		_room_shape[id] = shape


## Прямоугольная комната w×h: bbox + полная (нерезаная) форма.
func _set_room_box(id: int, dims: Vector2i) -> void:
	_room_cells[id] = dims
	_room_size[id] = Vector2(dims) * GRID
	_room_shape[id] = _rect_cells(Vector2i.ZERO, dims)


## Неравносторонний прямоугольник той же площади (~cw²): аспект из сида, ориентация случайна.
func _rect_dims(cw: int, id: int, aspect_max: float) -> Vector2i:
	var area := cw * cw
	var rng := RandomNumberGenerator.new()
	rng.seed = (_seed ^ (id * 40503)) + 1
	var aspect := rng.randf_range(1.0, aspect_max)
	var w := maxi(ROOM_MIN_CELLS, int(round(sqrt(float(area) * aspect))))
	var h := maxi(ROOM_MIN_CELLS, int(round(float(area) / float(w))))
	if rng.randf() < 0.5:
		var t := w
		w = h
		h = t
...
## Форма комнаты: ПО УМОЛЧАНИЮ прямоугольник (реальные помещения прямоугольны). Лишь с
## вероятностью IRREGULAR_CHANCE применяется один КОМПАКТНЫЙ модификатор (без «кишок»/тоннелей):
## скос углов / L-вырез / ниша — архитектурные, доступны всегда; крест / компактное пентамино —
## «совсем странные», только для НЕбольших комнат. Возвращает нормализованный набор клеток.
func _make_room_shape(cw: int, id: int) -> Dictionary:
	var rng := RandomNumberGenerator.new()
	rng.seed = _seed ^ (id * 2654435761)
	var base := _rect_dims(cw, id, ROOM_ASPECT_MAX)
	var rect := _rect_cells(Vector2i.ZERO, base)
	var area := base.x * base.y

	var mod := ROOM_SHAPE_FORCE
	if mod == "":
		# Нерегулярность — только для комнат ≥4×4: на мелких (2×2/3×3) срез легко съедает все клетки
		# (скос 2×2 убирает все 4 угла), оставляя пустую форму — title повисал бы без комнаты.
		if base.x < 4 or base.y < 4 or rng.randf() >= IRREGULAR_CHANCE:
			return rect
		var palette := ["скос", "угол", "ниша"]
		if area < BIG_AREA_CELLS:
			palette.append("крест")
			palette.append("пентамино")
		mod = palette[rng.randi() % palette.size()]

	var shape: Dictionary
	match mod:
		"скос":
			shape = _carve_octagon(base, maxi(1, int(round(float(mini(base.x, base.y)) * 0.2))))
		"угол":
			shape = _carve_L(base, rng)
		"ниша":
			shape = _carve_notch(base, rng)
		"крест":
			shape = _carve_plus(base)
		"пентамино":
			shape = _aggregate_pentominoes(area, rng)
		_:  # прямоугольник / неизвестное
			shape = rect
	# Страховка: вырожденный срез (почти пусто) — откатываемся к прямоугольнику, чтобы у комнаты
	# гарантированно были пол и стены (иначе её заголовок-вывеска висит в воздухе).
	if shape.size() < 3:
		shape = rect
	return _normalize_cells(shape)


## L-вырез: убираем один угловой прямоугольный блок (оставшиеся рукава ≥2 клеток в каждую сторону).
func _carve_L(cw: Vector2i, rng: RandomNumberGenerator) -> Dictionary:
	var out := _rect_cells(Vector2i.ZERO, cw)
	if cw.x < 3 or cw.y < 3:
		return out
	var bw := rng.randi_range(1, cw.x - 2)
	var bh := rng.randi_range(1, cw.y - 2)
	var x0 := 0 if rng.randi() % 2 == 0 else cw.x - bw
	var y0 := 0 if rng.randi() % 2 == 0 else cw.y - bh
...
## Ниша: прямоугольная выемка из СЕРЕДИНЫ одной стороны (по краям остаётся материал) — компактная
## вогнутость-альков, глубина ограничена, противоположная сторона цела ⇒ комната связна.
func _carve_notch(cw: Vector2i, rng: RandomNumberGenerator) -> Dictionary:
	var out := _rect_cells(Vector2i.ZERO, cw)
	var horizontal := rng.randi() % 2 == 0       # выемка с верх/низ (true) или лев/прав (false)
	var along := cw.x if horizontal else cw.y
	var perp := cw.y if horizontal else cw.x
	if along < 4 or perp < 3:
		return out
	@warning_ignore("integer_division")
	var nw := rng.randi_range(1, maxi(1, along - 3))      # ширина выемки вдоль стороны
	@warning_ignore("integer_division")
	var nd := rng.randi_range(1, maxi(1, perp / 2))       # глубина (меньше половины ⇒ есть цельная сторона)
	var s := rng.randi_range(1, along - 1 - nw)           # старт ≥1 ⇒ по краям остаётся материал
	var from_far := rng.randi() % 2 == 0                  # с дальней стороны (perp-1) или с ближней (0)
	for a in range(s, s + nw):
		for dd in nd:
			var p := (perp - 1 - dd) if from_far else dd
			var c := Vector2i(a, p) if horizontal else Vector2i(p, a)
			out.erase(c)
	return out
...
# --- Фаза 2.5 (ПРОТОТИП): форма коннекторов + правки кластера ---

## Комнаты уже получили полимино из пентамино (фаза 2). Здесь доводим форму там, где раньше
## размер был ещё неизвестен: коннектор (кроме кластер-якоря) — мягкий срез углов; комната,
## приклеенная к коннектору вплотную, — обратно в прямоугольник (склейке нужна прямая стена).
func _compute_room_shapes() -> void:
	if not SHAPE_ENABLE:
		return
	for id in _rooms.keys():
		if _direct_child.has(id):
			_room_shape[id] = _rect_cells(Vector2i.ZERO, _room_cells[id])
		elif _rooms[id]["kind"] == "connector" and not _is_cluster_anchor(id):
			_room_shape[id] = _carve_for(_room_cells[id], _pick_connector_shape(id))


## Архетип коннектора — только мягкие (углы), без сильных изгибов: квадрат/фаска/октагон.
func _pick_connector_shape(id: int) -> String:
	var palette := ["квадрат", "фаска", "октагон"]
	if SHAPE_FORCE != "":
		return SHAPE_FORCE
	var h: int = abs(_seed ^ (id * 2654435761))
	return palette[h % palette.size()]


## Чистая резка bbox cw по имени архетипа (без guard'ов) — для коннекторов и смоук-теста.
func _carve_for(cw: Vector2i, arch: String) -> Dictionary:
	var m: int = mini(cw.x, cw.y)
	match arch:
		"фаска":
			return _carve_octagon(cw, 1)
		"октагон":
			return _carve_octagon(cw, maxi(1, int(round(float(m) * 0.25))))
		"ромб":
			@warning_ignore("integer_division")
			return _carve_octagon(cw, maxi(1, (m - 1) / 2))
		"крест":
			return _carve_plus(cw)
		_:  # квадрат / неизвестное
			return _rect_cells(Vector2i.ZERO, cw)


## Срез четырёх углов треугольниками глубины d (манхэттен): октагон/ромб/фаска. Середины сторон
## остаются, поэтому периметр для дверей/объектов цел. d=1 — фаска (по клетке с угла).
func _carve_octagon(cw: Vector2i, d: int) -> Dictionary:
	var out: Dictionary = {}
	for x in cw.x:
		for y in cw.y:
			if x + y < d:                               # верх-лево
				continue
			if (cw.x - 1 - x) + y < d:                  # верх-право
				continue
			if x + (cw.y - 1 - y) < d:                  # низ-лево
				continue
			if (cw.x - 1 - x) + (cw.y - 1 - y) < d:     # низ-право
				continue
			out[Vector2i(x, y)] = true
	return out


## Плюс/крест: оставляем центральную вертикальную и горизонтальную полосы (~треть стороны),
## срезаем четыре угловых прямоугольника. Концы рукавов попадают в середины сторон bbox.
func _carve_plus(cw: Vector2i) -> Dictionary:
	var aw: int = maxi(1, int(ceil(float(cw.x) / 3.0)))
	var ah: int = maxi(1, int(ceil(float(cw.y) / 3.0)))
	@warning_ignore("integer_division")
	var x0: int = (cw.x - aw) / 2
	var x1: int = x0 + aw - 1
	@warning_ignore("integer_division")
	var y0: int = (cw.y - ah) / 2
	var y1: int = y0 + ah - 1
	var out: Dictionary = {}
	for x in cw.x:
		for y in cw.y:
			if (x >= x0 and x <= x1) or (y >= y0 and y <= y1):
				out[Vector2i(x, y)] = true
	return out


# --- Пентамино-агрегация: нерегулярный полимино комнаты ---

## Набирает ~area/5 пентамино и КОМПАКТНО стягивает их к центру: каждый следующий кусок
## приклеивается боком к фигуре, но из PENTO_TRIES случайных вариантов выбирается тот, что даёт
## наименьший bbox (минимум площади+вытянутости) — это убирает «кишки»/тоннели. Перекрытия
## сливаются, затем закрытые дырки заполняются полом. Форма нерегулярная, но цельная и плотная.
func _aggregate_pentominoes(area_cells: int, rng: RandomNumberGenerator) -> Dictionary:
	var n: int = maxi(1, int(ceil(float(area_cells) / 5.0)))
	var blob: Dictionary = {}
	for c in _rand_pentomino(rng):
		blob[c] = true
	for _i in range(1, n):
		var piece: Array = _rand_pentomino(rng)
		var best_off := Vector2i.ZERO
		var best_score := INF
		for _t in PENTO_TRIES:
			var cells: Array = blob.keys()
			var anchor: Vector2i = cells[rng.randi() % cells.size()]
			var target: Vector2i = anchor + _dirs[rng.randi() % _dirs.size()]
			var pc: Vector2i = piece[rng.randi() % piece.size()]
			var off: Vector2i = target - pc
			var score := _placement_score(blob, piece, off)
			if score < best_score:
				best_score = score
				best_off = off
		for c in piece:
			blob[c + best_off] = true
	_fill_holes(blob)
	return _normalize_cells(blob)


## Оценка размещения куска (меньше — компактнее): площадь bbox объединения плюс штраф за
## вытянутость (разница сторон) ⇒ предпочитаем близкие к квадрату плотные раскладки.
func _placement_score(blob: Dictionary, piece: Array, off: Vector2i) -> float:
	var mnx := 1 << 30
	var mny := 1 << 30
	var mxx := -(1 << 30)
	var mxy := -(1 << 30)
	for c in blob:
		mnx = mini(mnx, c.x); mny = mini(mny, c.y)
		mxx = maxi(mxx, c.x); mxy = maxi(mxy, c.y)
	for c in piece:
	...
	var w := mxx - mnx + 1
	var h := mxy - mny + 1
	return float(w * h) + float(absi(w - h)) * float(GRID)


## Случайное пентамино из набора, повёрнутое 0..3 раза на 90° (нормализованное в неотрицательные координаты).
func _rand_pentomino(rng: RandomNumberGenerator) -> Array:
	var pieces := _pentominoes()
	var piece: Array = pieces[rng.randi() % pieces.size()]
	var rot: int = rng.randi() % 4
	for _r in rot:
		piece = _rotate_cells(piece)
	return piece


## 12 свободных пентамино (F I L N P T U V W X Y Z) как списки Vector2i.
func _pentominoes() -> Array:
	return [
...
## Поворот набора клеток на 90° ((x,y)->(-y,x)) с нормализацией в неотрицательные координаты.
func _rotate_cells(cells: Array) -> Array:
	var out: Array = []
	for c in cells:
...
## Сдвигает список клеток так, чтобы min по обеим осям был 0.
func _normalize_list(cells: Array) -> Array:
	var mnx := 1 << 30
	var mny := 1 << 30
	for c in cells:
		mnx = mini(mnx, c.x)
	...
	var out: Array = []
	for c in cells:
		out.append(Vector2i(c.x - mnx, c.y - mny))
	return out


## То же для множества-словаря: возвращает новый словарь со сдвигом min->0.
func _normalize_cells(cells: Dictionary) -> Dictionary:
	var mnx := 1 << 30
	var mny := 1 << 30
	for c in cells:
		mnx = mini(mnx, c.x)
	...
	var out: Dictionary = {}
	for c in cells:
		out[Vector2i(c.x - mnx, c.y - mny)] = true
	return out


## bbox-размер множества клеток в клетках (ширина, высота).
func _bbox_of(cells: Dictionary) -> Vector2i:
	var mxx := 0
	var mxy := 0
	for c in cells:
		mxx = maxi(mxx, c.x)
		mxy = maxi(mxy, c.y)
	return Vector2i(mxx + 1, mxy + 1)


## Заполняет полом закрытые дырки (пустые клетки, не достижимые из-за пределов bbox флуд-филлом
## по 4 соседям). Так внутри комнаты не остаётся недостижимых колодцев.
func _fill_holes(blob: Dictionary) -> void:
	var norm := _normalize_cells(blob)
	var bb := _bbox_of(norm)
	# Внешняя заливка по рамке [-1 .. bb] (с запасом в 1 клетку, чтобы обойти фигуру снаружи).
	var outside: Dictionary = {}
	var stack: Array = [Vector2i(-1, -1)]
	while not stack.is_empty():
		var c: Vector2i = stack.pop_back()
		if c.x < -1 or c.y < -1 or c.x > bb.x or c.y > bb.y:
			continue
		if outside.has(c) or norm.has(c):
			continue
		outside[c] = true
		for d in _dirs:
			stack.append(c + d)
	# Любая клетка bbox, не входящая в фигуру и не достижимая снаружи, — дырка: заполняем.
	for x in bb.x:
		for y in bb.y:
			var c := Vector2i(x, y)
			if not norm.has(c) and not outside.has(c):
				norm[c] = true
	blob.clear()
	for c in norm:
		blob[c] = true
...
func _fill_occupancy() -> void:
	for id in _rooms.keys():
		var c: Vector2i = _room_cell[id]
		var cw: Vector2i = _room_cells[id]
		# _occ — только клетки пола (для контура стен/проёмов). _bbox_cells — весь bbox комнаты
		# (включая срезанные клетки): коридор не должен пересекать комнату даже через её вырез.
		for dx in cw.x:
			for dy in cw.y:
				_bbox_cells[Vector2i(c.x + dx, c.y + dy)] = true
		for lc in _room_shape[id]:
			_occ[Vector2i(c.x + lc.x, c.y + lc.y)] = id
...
func _passable(cell: Vector2i, allow_corr: bool) -> bool:
	# Любая клетка bbox комнаты (включая срезанную) непроходима — коридор идёт только по улицам,
	# не пересекая комнату через её вырез/нишу.
	if _bbox_cells.has(cell):
		return false
...
## Клетки-стены комнаты (контур полимино): клетки пола, у которых хотя бы один из 4 соседей
## не принадлежит этой комнате (выходит наружу/в срезанный угол).
func _border_cells(id: int) -> Array:
	var c: Vector2i = _room_cell[id]
	var out: Array = []
	for lc in _room_shape[id]:
		var cell := Vector2i(c.x + lc.x, c.y + lc.y)
		for d in _dirs:
			if _occ.get(cell + d, CORR - 1) != id:
				out.append(cell)
				break
	return out
...
func _add_opening(id: int, room_cell: Vector2i, d: Vector2i) -> Dictionary:
	...
	# Координата плоскости стены проёма (для вывески-заголовка): на нерегулярной форме стена с
	# проёмом может быть утоплена внутрь bbox, поэтому считаем её по реальной клетке, а не по ±half.
	var lx := room_cell.x - c0.x
	var ly := room_cell.y - c0.y
	var half_x := cw.x * GRID * 0.5
	var half_z := cw.y * GRID * 0.5
	var off := _edge_off({"key": key}, lx if d.x != 0 else ly, half_x, half_z)
	var op := {"key": key, "lo": lo, "hi": lo + GRID, "off": off}
...
## Клетка-стена комнаты в направлении d: самая крайняя клетка ПОЛА вдоль d (для пентамино
## середина стороны bbox может оказаться пустой, поэтому ищем по реальным клеткам формы).
func _border_toward(id: int, d: Vector2i) -> Vector2i:
	var c: Vector2i = _room_cell[id]
	var best: Vector2i = Vector2i.ZERO
	var best_score := -2147483647
	for lc in _room_shape[id]:
		var score: int = lc.x * d.x + lc.y * d.y
		if score > best_score:
			best_score = score
			best = lc
	return c + best
...
func _build_room(id: int, parent: Node3D, on_transition: Callable) -> void:
	...
	_build_floor(holder, id, floor_color)
...
## Пол — по одному GRID-квадрату на каждую клетку полимино (срезанные углы остаются без пола).
func _build_floor(holder: Node3D, id: int, floor_color: Color) -> void:
	var size: Vector2 = _room_size[id]
	var half_x := size.x * 0.5
	var half_z := size.y * 0.5
	for lc in _room_shape[id]:
		var cx: float = (lc.x + 0.5) * GRID - half_x
		var cz: float = (lc.y + 0.5) * GRID - half_z
		_add_box(holder, Vector3(GRID, 0.4, GRID), Vector3(cx, -0.2, cz), floor_color, true)


## Стены обходят контур полимино: для каждой клетки пола каждое из 4 рёбер, смотрящее на
## не-пол (наружу/срезанный угол), становится сегментом стены длиной GRID. Ребро-проём
## (совпадает с интервалом _room_openings) пропускаем — это дверь.
func _build_walls(holder: Node3D, id: int, color: Color) -> void:
	var shape: Dictionary = _room_shape[id]
	var size: Vector2 = _room_size[id]
	var half_x := size.x * 0.5
	var half_z := size.y * 0.5
	...
	for lc in shape:
		var x_lo: float = lc.x * GRID - half_x
		var x_hi: float = (lc.x + 1) * GRID - half_x
		var z_lo: float = lc.y * GRID - half_z
		var z_hi: float = (lc.y + 1) * GRID - half_z
		var x_center: float = (x_lo + x_hi) * 0.5
		var z_center: float = (z_lo + z_hi) * 0.5
		# E (+x) -> стена px; вдоль неё координата z
		if not shape.has(lc + Vector2i(1, 0)):
			_edge_wall(holder, ops, "px", z_center, x_hi, z_lo, z_hi, h, color)
		# W (-x) -> nx
		if not shape.has(lc + Vector2i(-1, 0)):
			_edge_wall(holder, ops, "nx", z_center, x_lo, z_lo, z_hi, h, color)
		# S (+z) -> pz; вдоль неё координата x
		if not shape.has(lc + Vector2i(0, 1)):
			_edge_wall(holder, ops, "pz", x_center, z_hi, x_lo, x_hi, h, color)
		# N (-z) -> nz
		if not shape.has(lc + Vector2i(0, -1)):
			_edge_wall(holder, ops, "nz", x_center, z_lo, x_lo, x_hi, h, color)


## Граничное ребро клетки: полная стена, либо — если ребро это проём — только перемычка над
## дверью (от DOOR_HEIGHT до верха). Так над проходом всегда есть полотно: на нём держится
## вывеска-заголовок (не висит в воздухе), а сам проход остаётся высотой с дверь.
func _edge_wall(holder: Node3D, ops: Array, key: String, along_center: float, edge_off: float,
		along_lo: float, along_hi: float, h: float, color: Color) -> void:
	if _edge_is_opening(ops, key, along_lo, along_hi):
		if h > DOOR_HEIGHT + 0.05:
			_add_edge_wall(holder, key, along_center, edge_off, DOOR_HEIGHT, h, color)   # перемычка
	else:
		_add_edge_wall(holder, key, along_center, edge_off, 0.0, h, color)


## Совпадает ли ребро [lo,hi] (вдоль стены key) с зарегистрированным проёмом (дверью).
## Проёмы GRID-выровнены и GRID-широкие, поэтому сверяем середину ребра с интервалом проёма.
func _edge_is_opening(ops: Array, key: String, lo: float, hi: float) -> bool:
	var mid: float = (lo + hi) * 0.5
	for o in ops:
		if o["key"] == key and float(o["lo"]) - 0.01 <= mid and mid <= float(o["hi"]) + 0.01:
			return true
	return false


## Сегмент стены длиной GRID на ребре клетки от высоты y0 до y1. edge_off — координата ребра по
## нормали стены; along_center — центр ребра вдоль стены. Стену сдвигаем внутрь на полтолщины ⇒
## внешняя грань ложится точно на ребро (стыки соседних клеток грань-в-грань, без дырок/z-fight).
func _add_edge_wall(holder: Node3D, key: String, along_center: float, edge_off: float,
	...
	match key:
		"px":
			_add_box(holder, Vector3(t, hh, GRID), Vector3(edge_off - t * 0.5, cy, along_center), color, true)
		"nx":
			_add_box(holder, Vector3(t, hh, GRID), Vector3(edge_off + t * 0.5, cy, along_center), color, true)
		"pz":
			_add_box(holder, Vector3(GRID, hh, t), Vector3(along_center, cy, edge_off - t * 0.5), color, true)
		"nz":
			_add_box(holder, Vector3(GRID, hh, t), Vector3(along_center, cy, edge_off + t * 0.5), color, true)
...
func _place_objects(room: Dictionary, holder: Node3D, on_transition: Callable, skip_obj = null) -> void:
	...
	for obj in objs:
		...
		# Курсор + центр; при переполнении (объектов больше, чем стен) клампим, чтобы объект не
		# вылезал за стену наружу — лучше лёгкое перекрытие в конце, чем «висит снаружи комнаты».
		var u: float = clampf(span["cursor"] + w * 0.5, w * 0.5, maxf(w * 0.5, span["length"] - w * 0.5))
...
func _build_room_title(holder: Node3D, id: int, title_obj: Dictionary) -> bool:
	...
	# Координата стены проёма: реальная (утопленная) плоскость, а не край bbox — иначе на
	# нерегулярной форме вывеска повисает в воздухе. Старые проёмы без "off" — фолбэк на ±half.
	var off: float = float(op.get("off", half_x if (key == "px" or key == "nx") else half_z))
	...
	# Вывеска живёт в зоне перемычки над дверью [DOOR_HEIGHT..верх] — там есть полотно стены,
	# поэтому она не висит в воздухе (см. _edge_wall: над проёмом остаётся стена-перемычка).
	var lintel: float = maxf(0.1, wall_h - DOOR_HEIGHT)
	var band_h := clampf(glyph_m * 1.5 + 0.2, minf(TITLE_MIN_H, lintel), minf(TITLE_MAX_H, lintel))
	var band_w := opening_w + 2.0 * TITLE_OVERHANG
	var center_y := (DOOR_HEIGHT + wall_h) * 0.5   # по центру перемычки
	...
	match key:
		"px":
			pos = Vector3(off, center_y, along); out_n = Vector3(1, 0, 0); inward_yaw = -PI * 0.5
		"nx":
			pos = Vector3(off, center_y, along); out_n = Vector3(-1, 0, 0); inward_yaw = PI * 0.5
		"pz":
			pos = Vector3(along, center_y, off); out_n = Vector3(0, 0, 1); inward_yaw = PI
		_:  # nz
			pos = Vector3(along, center_y, off); out_n = Vector3(0, 0, -1); inward_yaw = 0.0
...
## Пролёты под объекты — по 4 ВНЕШНИМ стенам bbox (порядок чтения по часовой), за вычетом
## проёмов и срезанных участков (_carved_gaps). Комнаты теперь в основном прямоугольные, поэтому
## внешних стен достаточно, а объекты стоят ровно и не лезут на утопленные/тонкие стены вырезов.
func _wall_spans(id: int) -> Array:
	...
	for w in _object_walls():
		var is_x: bool = w["kind"] == "x"
		var off_half: float = half_x if is_x else half_z
		var along_half: float = half_z if is_x else half_x
		var plane: float = w["sign"] * (off_half - OBJECT_INSET)   # плоскость объектов (внутрь от стены)
		var lo := -along_half + CORNER_MARGIN
		var hi := along_half - CORNER_MARGIN
		var gaps: Array = []
		for o in ops:
			if o["key"] == w["key"]:
				gaps.append([o["lo"], o["hi"]])
		# На срезанных участках внешней стены полотна нет — туда объекты не вешаем.
		for cg in _carved_gaps(id, w):
			gaps.append(cg)
		gaps.sort_custom(func(p, q): return p[0] < q[0])
		var free := _subtract_gaps(lo, hi, gaps, DOOR_MARGIN)
		if w["flip"]:
			free.reverse()
		for iv in free:
			if iv[1] - iv[0] >= 0.6:
				spans.append(_mk_span(w, plane, iv[0], iv[1]))
	return spans


## Интервалы вдоль внешней стены w, где краевая клетка bbox срезана (нет пола) — там нет полотна
## стены. [lo,hi] в координатах вдоль стены (центр = 0), как у проёмов.
func _carved_gaps(id: int, w: Dictionary) -> Array:
	var shape: Dictionary = _room_shape[id]
	var cw: Vector2i = _room_cells[id]
	var size: Vector2 = _room_size[id]
	var out: Array = []
	if w["kind"] == "x":
		var lx: int = (cw.x - 1) if w["sign"] > 0.0 else 0
		var half_z := size.y * 0.5
		for ly in cw.y:
			if not shape.has(Vector2i(lx, ly)):
				out.append([ly * GRID - half_z, (ly + 1) * GRID - half_z])
	else:
		var ly: int = (cw.y - 1) if w["sign"] > 0.0 else 0
		var half_x := size.x * 0.5
		for lx in cw.x:
			if not shape.has(Vector2i(lx, ly)):
				out.append([lx * GRID - half_x, (lx + 1) * GRID - half_x])
	return out


## Нормали-стены в порядке обхода ПО ЧАСОВОЙ (вид сверху, север = -Z): nz→px→pz→nx. `sign`/`yaw`/
## `key` совпадают с геометрией стен (_add_edge_wall); `flip` — стены, где «слева направо» для
## зрителя идёт в сторону убывания координаты вдоль стены.
...
## Координата плоскости стены (x для ±x-стен, z для ±z-стен) для линии lk: внешняя грань
## клетки lk по нормали стены. lk — индекс клетки вдоль нормали.
func _edge_off(w: Dictionary, lk: int, half_x: float, half_z: float) -> float:
	match w["key"]:
		"px":
			return (lk + 1) * GRID - half_x
		"nx":
			return lk * GRID - half_x
		"pz":
			return (lk + 1) * GRID - half_z
		_:  # nz
			return lk * GRID - half_z
...
## Пролёт стены под укладку объектов. Курсор u ∈ [0, length] от ЛЕВОГО для зрителя края:
## координата вдоль стены t = base_t + sign_u·u. У flip-стен лево — высокий конец координаты
## (base_t = hi, sign_u = -1), у обычных — низкий. `plane` — локальная координата плоскости
## объектов (уже с отступом OBJECT_INSET внутрь от стены).
func _mk_span(w: Dictionary, plane: float, lo: float, hi: float) -> Dictionary:
	var base_t: float = hi if w["flip"] else lo
	var sign_u: float = -1.0 if w["flip"] else 1.0
	return {"w": w, "plane": plane, "base_t": base_t, "sign_u": sign_u, "length": hi - lo, "cursor": 0.0}


func _span_pos(span: Dictionary, u: float) -> Vector3:
	var w: Dictionary = span["w"]
	var plane: float = span["plane"]
	var t: float = span["base_t"] + span["sign_u"] * u
	if w["kind"] == "x":
		return Vector3(plane, 0.0, t)
	return Vector3(t, 0.0, plane)
