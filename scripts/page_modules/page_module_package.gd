...
static func unpack(module: Dictionary) -> Dictionary:
	...
	if not PageModuleCache.valid_hash(hash):
		return _error("invalid_hash")
	var archive_path := Sandbox.resolve(str(module.get("cache_path", PageModuleCache.path_for(hash))))
	...
	for path in files:
		...
		if not PageModuleManifest.valid_module_path(checked_path) or seen.has(checked_path) \
	...
	reader.close()
	var parsed := PageModuleManifest.parse(contents[MANIFEST], str(module.get("id", "")))
