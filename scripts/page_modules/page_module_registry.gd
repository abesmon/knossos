...
func prepare(definitions: Array, mode: ScriptMode,
	...
	for definition in definitions:
		...
		if kind not in ["inline", "script", "package"]:
			continue
		if str(module.get("runtime", "")) != PageModuleCollector.RUNTIME_TRUSTED:
		...
		_modules[str(module.get("id", ""))] = {
			"definition": module,
			"session": PageModuleSession.new(str(module.get("id", "")), hash),
...
func _prepare_package(module: Dictionary) -> String:
	...
	_modules[module_id] = {"definition": module, "exports": exports,
		"session": PageModuleSession.new(module_id, str(module.get("hash", "")))}
	return ""
...
func instantiate_export(module_id: String, export_name: String) -> Dictionary:
	...
	var session: PageModuleSession = _modules[module_id].session
	var manifest: Dictionary = definition.get("manifest", {})
	var context := PageModuleContext.new(module_id, str(definition.get("hash", "")), node, session,
...
func _bind_lifecycle(node: Node, context: PageModuleContext) -> void:
	var on_ready := func():
