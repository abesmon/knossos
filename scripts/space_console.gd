...
var _flush_note := ""          # приписка к итогу флаша (например, пропущенные объекты с файлами)
var _timeout: Timer
...
func _on_flush() -> void:
	...
	# Realtime-ресурсы (vrwebblob://) в дельте: блобы уходят документной формой
	# (kind="vrweb-blob", base64) первыми в пакете — но только если сервер их принимает
	# (capability accepts_kinds). Старому серверу пакет уходит байт-в-байт прежнего формата,
	# а объекты со ссылками на блобы пропускаются — иначе в страницу запеклись бы мёртвые
	# ссылки. См. docs/network/realtime-resources.md.
	var cap := {}
	var refs := {}
	for o in objects:
		_collect_blob_refs(o.get("props", {}), refs)
	if not refs.is_empty():
		_set_status("Проверяем, принимает ли сервер файлы…")
		cap = await _fetch_capability(endpoint)
		if not is_instance_valid(self) or not _flushing:
			return
		var blobs: Array = []
		var missing := {}   # hex без локальных байтов (не докачали у пиров) — объект не уйдёт
		if (cap.get("accepts_kinds", []) as Array).has("vrweb-blob"):
			for hex in refs:
				var bytes: PackedByteArray = BlobStore.bytes_by_hex(str(hex))
				if bytes.is_empty():
					missing[hex] = true
				else:
					blobs.append({"id": "blob:" + str(hex), "kind": "vrweb-blob", "parent": "",
						"props": {"algo": "sha256", "hash": str(hex),
							"data": Marshalls.raw_to_base64(bytes)}})
		else:
			missing = refs   # сервер файлов не принимает — все blob-объекты мимо
		if not missing.is_empty():
			var before := objects.size()
			objects = objects.filter(func(o) -> bool:
				var r := {}
				_collect_blob_refs(o.get("props", {}), r)
				for hex in r:
					if missing.has(hex):
						return false
				return true)
			var reason := "сервер не принимает файлы" if missing.size() == refs.size() \
					else "байты ещё не докачаны у пиров"
			_flush_note = " Пропущено объектов с файлами: %d (%s)." % [before - objects.size(), reason]
			if objects.is_empty():
				_finish_flush(false, "Сохранять нечего: вся дельта — объекты с файлами (%s)" % reason)
				return
	...
	if auth != "":
		...
		if sig == "":
			_finish_flush(false, "Не удалось подписать запрос ключом идентичности")
			return
		var cp: Array = HomeServer.certificate_payload()
		body["identity"] = {"certificate_json": cp[0], "signature": cp[1]}
		body["proof_signature"] = sig
	else:
		_finish_flush(false, "Флаш требует идентичности: войдите на домашний сервер (вкладка «Аккаунт»)")
		return

	# Анонсированный лимит тела: с вшитыми файлами его легко превысить — честный отказ
	# заранее лучше серверного 413.
	var body_str := JSON.stringify(body)
	var cap_max := int(cap.get("max_bytes", 0))
	if cap_max > 0 and body_str.length() > cap_max:
		_finish_flush(false, "Дельта с файлами больше лимита сервера (%d КБ) — уберите часть изображений" % int(cap_max / 1024.0))
		return
	...
	req.timeout = 10.0
	add_child(req)
	var err := req.request(endpoint, headers, HTTPClient.METHOD_POST, body_str)
	if err != OK:
		req.queue_free()
...
func _finish_flush(ok: bool, message: String) -> void:
	...
	_lock(false)
	_mark_error(not ok)
	_set_status(message + _flush_note)
	_flush_note = ""


## Собирает hex-хэши realtime-ресурсов (vrwebblob://) из всех строковых значений props —
## рекурсивно по словарям/массивам (attrs у vrweb-node, set у vrweb-patch).
static func _collect_blob_refs(value, out: Dictionary) -> void:
	match typeof(value):
		TYPE_STRING:
			var hex := BlobProtocol.hex_of(str(value))
			if hex != "":
				out[hex] = true
		TYPE_DICTIONARY:
			for k in value:
				_collect_blob_refs(value[k], out)
		TYPE_ARRAY:
			for v in value:
				_collect_blob_refs(v, out)


## Capability-запрос persist-endpoint'а (GET ?url=…, см. docs/page-persistence.md).
## {} — сервер не ответил/ответ не JSON: трактуется как «файлы не принимает».
func _fetch_capability(endpoint: String) -> Dictionary:
	var req := HTTPRequest.new()
	req.timeout = 8.0
	add_child(req)
	var url := endpoint + ("&" if endpoint.contains("?") else "?") \
			+ "url=" + str(_get_page_url.call()).uri_encode()
	if req.request(url, PackedStringArray(["Accept: application/json"])) != OK:
		req.queue_free()
		return {}
	var res: Array = await req.request_completed
	req.queue_free()
