...
static func parse(bytes: PackedByteArray, declared_id: String = "") -> Dictionary:
	...
	var runtime := str(raw.get("runtime", ScriptingModuleCollector.RUNTIME_TRUSTED))
	if not ScriptingModuleCollector._valid_runtime(runtime):
	...
	var normalized := {
		...
		"permissions": normalized_permissions, "requires": normalized_requires,
