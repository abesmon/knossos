class_name ScriptingModuleFetcher
extends Node

## Async fetch + integrity + immutable cache. Не компилирует и не распаковывает модули.

const MAX_CONCURRENT := 4
const MAX_BYTES := ScriptingModuleCache.MAX_ARTIFACT_BYTES
...
## on_done({modules, errors, cancelled}). Definitions — IR ScriptingModuleCollector.
...
func _accept_bytes(module: Dictionary, resolved_url: String, bytes: PackedByteArray,
	...
	var checked := ScriptingModuleIntegrity.verify(module, _page_url, resolved_url, bytes, previous_hash)
