class_name ScriptingModuleStateAPI
extends RefCounted

## Module-namespaced facade Replicated State. Compatibility boundary для scripting modules.
...
static func _valid_local_id(value: String) -> bool:
	return ScriptingModuleCollector._id_error(value).is_empty()
