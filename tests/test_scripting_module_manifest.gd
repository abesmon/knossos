...
func _initialize() -> void:
	...
	_eq(result.manifest.requires, ["vrweb/core/1", "godot/engine/4"],
	...
	_eq(ScriptingModuleManifest.valid_module_path("scenes/main.tscn"), true, "module-local path accepted")
	_eq(ScriptingModuleManifest.valid_module_path("res://client.gd"), false, "res path rejected")
	_eq(ScriptingModuleManifest.valid_module_path("a/../client.gd"), false, "traversal rejected")
	var duplicate_capability := JSON.stringify({
		"format": 1, "id": "acme.duplicate", "runtime": "trusted-gdscript",
		"exports": {"default": {"script": "main.gd"}},
