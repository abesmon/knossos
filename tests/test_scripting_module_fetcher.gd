...
func _ready() -> void:
	...
	var collected := ScriptingModuleCollector.collect(doc, "vrwebresource://external_script.html")
	fetcher.fetch_all(collected.modules, "vrwebresource://external_script.html", func(result):
		...
		if result.modules.size() == 1:
			...
			var prepared := registry.prepare(result.modules, ScriptingModuleRegistry.ScriptMode.ALLOW_ALL)
			...
			if made.node != null:
				_eq(made.node.call("answer"), 73, "external module-defined method executes")
				made.node.free()
			...
			if component != null:
				...
				_eq(component.call("presentation_text"), "EXTERNAL SCRIPT: from-html = 73",
						"scene presentation uses external module-defined behavior")
