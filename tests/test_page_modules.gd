...
func _test_inline() -> void:
	...
	var result := PageModuleCollector.collect(HtmlParser.parse(html), "https://example.test/world/index.html")
...
"""
	var result := PageModuleCollector.collect(HtmlParser.parse(html), "https://example.test/")
